...
class OP(Source):
    ...
    def get_vector(self, extent, EPSG):
        ...
        self.download_data(url, path, "ČUZK Ruian OP")
        path = '/vsizip/' + os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'SPH_SHP_JTSK.zip') + '/JTSK/SPH_OP.shp'
        vector = QgsVectorLayer(path, "OP", "ogr")
