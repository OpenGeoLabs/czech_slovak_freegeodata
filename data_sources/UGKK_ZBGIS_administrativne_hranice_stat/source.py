...
class Stat(Source):
    ...
    def get_vector(self, extent, EPSG):
        ...
        self.download_data(url, path, "ZBGIS - administratívne hranice SR - SR")
        path = '/vsizip/' + os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'ah_gpkg_0.zip') + '/USJ_hranice_0.gpkg|layername=sr_0'
        vector = QgsVectorLayer(path, "ZBGIS - administratívne hranice SR - SR", "ogr")
